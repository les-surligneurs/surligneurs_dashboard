...
class  Article:
    ...
    def affichage_complet(self , titre :str  ) -> list ():
        ...
        row5 = html.Tr([html.Td("Url : "), html.Td(dcc.Link (href =info["url"][0]))])


        info = pd.read_sql_query("  select  * from resnlp where titrecomm =  \'" + str (titre)  +"\' ;" , self.cnt.conn )
        #pour voir ce que recupére la requete directement et exclure l'eventualité d'un probleme d'affichage
        #print(info["namedentities"])
